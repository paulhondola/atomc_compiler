...
int main() {
  ...
  // Loop iterates for i = 0, 1 (n = 2, step = 1; exits when i == n)
  ...
  // Integer opcodes must appear in the trace
  assert(strstr(trace, "PUSH.integer_value") != NULL);
  printf(GREEN "Test 4 (PUSH.integer_value appears in trace) passed\n" RESET);

  assert(strstr(trace, "ADD.integer_value") != NULL);
  printf(GREEN "Test 5 (ADD.integer_value appears in trace) passed\n" RESET);

  assert(strstr(trace, "LESS.integer_value") != NULL);
  printf(GREEN "Test 6 (LESS.integer_value appears in trace) passed\n" RESET);

  // Float opcodes must NOT appear — this is an integer program
  assert(strstr(trace, "PUSH.f") == NULL);
  printf(GREEN "Test 7 (no PUSH.f — integer program only) passed\n" RESET);

  assert(strstr(trace, "HALT") != NULL);
  printf(GREEN "Test 8 (trace ends with HALT) passed\n" RESET);
